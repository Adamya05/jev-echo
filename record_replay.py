"""Record the replay shown on the page: all three players start from board 0.
...
ap.add_argument("--budget", type=int, default=181,
                help="Echo + search ms per move; the tree uses somewhat less")
ap.add_argument("--reuse-jev", action="store_true")
a = ap.parse_args()

out = pathlib.Path("results/marathon_crash.json")
jev = (json.loads(out.read_text())["jev"][0] if a.reuse_jev
       else one_game(search.build("jev_raw", None), 0, CAP))
srch = one_game(search.build(f"board_tree_{a.budget}", None), 0, CAP)
jev["t0"] = srch["t0"] = 0.0
